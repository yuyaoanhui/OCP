...
import com.futuredata.judicature.plugin.mybatis.generator.utils.PluginTools;
...
public class SelectiveEnhancedPlugin extends BasePlugin {
  ...
  public boolean validate(List<String> warnings) {
    ...
    // 插件位置
    PluginTools.shouldAfterPlugins(getContext(), this.getClass(), warnings, MysqlInsertOrUpdatePlugin.class);
  ...
  public boolean modelBaseRecordClassGenerated(TopLevelClass topLevelClass,
    ...
    // field
    Field selectiveColumnsField =
        new Field("selectiveColumns", new FullyQualifiedJavaType("Map<String, Boolean>"));
    ...
    mHasSelective1
        .addParameter(new Parameter(FullyQualifiedJavaType.getStringInstance(), "column"));
    ...
    mSelective.addParameter(
        new Parameter(new FullyQualifiedJavaType(ModelColumnPlugin.ENUM_NAME), "columns", true));
    ...
    for (Element ele : element.getElements()) {
      ...
      if (ele instanceof XmlElement) {
        ...
        ifEle.addAttribute(new Attribute("test",
            prefix + METHOD_HAS_SELECTIVE + "(\'" + column.getActualColumnName() + "\')"));
   ...
   * @param allColumns
   */
  private void replaceEleForUpsertByExampleSelective(XmlElement element, String prefix,
      IntrospectedTable introspectedTable, boolean allColumns) {
    // choose
    XmlElement chooseEle = new XmlElement("choose");
    // when
    XmlElement whenEle = new XmlElement("when");
    whenEle.addAttribute(new Attribute("test", prefix + METHOD_HAS_SELECTIVE + "()"));
    for (IntrospectedColumn introspectedColumn : (allColumns ? introspectedTable.getAllColumns()
        : introspectedTable.getNonBLOBColumns())) {
      XmlElement eleIf = new XmlElement("if");
      eleIf.addAttribute(new Attribute("test", prefix + METHOD_HAS_SELECTIVE + "(\'"
