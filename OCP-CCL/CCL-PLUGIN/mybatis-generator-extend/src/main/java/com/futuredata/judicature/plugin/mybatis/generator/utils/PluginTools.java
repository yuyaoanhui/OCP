...
public class PluginTools {
   ...
   * @return
   */
  public static boolean shouldAfterPlugins(Context context, Class<?> plugin, List<String> warnings,
      Class<?>... plugins) {
    int index = getPluginIndex(context, plugin);
    if (plugins != null) {
      for (Class<?> cls : plugins) {
        ...
        if (index1 != -1 && index1 >= index) {
          warnings.add("itfsw:插件" + plugin.getTypeName() + "插件建议配置在插件" + cls.getTypeName()
              + "后面，否则某些功能可能得不到增强！");
          return false;
        }
      }
