...
public class LimitPlugin extends BasePlugin {
  ...
  public boolean validate(List<String> warnings) {
    ...
    if ("com.mysql.jdbc.Driver".equalsIgnoreCase(
      ...
      warnings.add("mybatis:插件" + this.getClass().getTypeName() + "只支持MySQL数据库！");
      return false;
    }
  ...
  public boolean modelExampleClassGenerated(TopLevelClass topLevelClass,
    ...
    // 添加offset和rows字段
    Field offsetField = JavaElementGeneratorTools.generateField("offset", JavaVisibility.PROTECTED,
        integerWrapper, null);
    commentGenerator.addFieldComment(offsetField, introspectedTable);
    topLevelClass.addField(offsetField);

    Field rowsField = JavaElementGeneratorTools.generateField("rows", JavaVisibility.PROTECTED,
        integerWrapper, null);
    commentGenerator.addFieldComment(rowsField, introspectedTable);
    topLevelClass.addField(rowsField);
    logger.debug("mybatis(MySQL分页插件):" + topLevelClass.getType().getShortName() + "增加offset和rows字段");
    ...
    logger.debug("mybatis(MySQL分页插件):" + topLevelClass.getType().getShortName()
        + "增加offset和rows的getter和setter实现。");

    // 提供几个快捷方法
    Method setLimit = JavaElementGeneratorTools.generateMethod("limit", JavaVisibility.PUBLIC,
        topLevelClass.getType(), new Parameter(integerWrapper, "rows"));
    commentGenerator.addGeneralMethodComment(setLimit, introspectedTable);
    setLimit =
        JavaElementGeneratorTools.generateMethodBody(setLimit, "this.rows = rows;", "return this;");
    topLevelClass.addMethod(setLimit);

    Method setLimit2 = JavaElementGeneratorTools.generateMethod("limit", JavaVisibility.PUBLIC,
        topLevelClass.getType(), new Parameter(integerWrapper, "offset"),
        new Parameter(integerWrapper, "rows"));
    commentGenerator.addGeneralMethodComment(setLimit2, introspectedTable);
    setLimit2 = JavaElementGeneratorTools.generateMethodBody(setLimit2, "this.offset = offset;",
        "this.rows = rows;", "return this;");
    topLevelClass.addMethod(setLimit2);
    logger.debug("mybatis(MySQL分页插件):" + topLevelClass.getType().getShortName() + "增加limit方法。");

    Method setPage = JavaElementGeneratorTools.generateMethod("page", JavaVisibility.PUBLIC,
        topLevelClass.getType(), new Parameter(integerWrapper, "page"),
        new Parameter(integerWrapper, "pageSize"));
    commentGenerator.addGeneralMethodComment(setPage, introspectedTable);
    setPage = JavaElementGeneratorTools.generateMethodBody(setPage,
        "this.offset = page * pageSize;", "this.rows = pageSize;", "return this;");
    topLevelClass.addMethod(setPage);
    logger.debug("mybatis(MySQL分页插件):" + topLevelClass.getType().getShortName() + "增加page方法");
  ...
  public void generateLimitElement(XmlElement element, IntrospectedTable introspectedTable) {
    ...
    logger.debug("mybatis(MySQL分页插件):" + introspectedTable.getMyBatis3XmlMapperFileName()
