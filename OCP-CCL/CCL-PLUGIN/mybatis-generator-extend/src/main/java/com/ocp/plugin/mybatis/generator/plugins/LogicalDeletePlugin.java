...
import com.ocp.plugin.mybatis.generator.utils.FormatTools;
...
public class LogicalDeletePlugin extends BasePlugin {
  ...
  public static final String METHOD_SELECT_BY_PRIMARY_KEY_WITH_LOGICAL_DELETE =
      "selectByPrimaryKeyWithLogicalDelete"; // selectByPrimaryKey 的逻辑删除增强
  ...
  public void initialized(IntrospectedTable introspectedTable) {
    ...
    if (introspectedTable.getTableConfigurationProperty(PRO_LOGICAL_DELETE_COLUMN) != null) {
      logicalDeleteColumn =
          introspectedTable.getTableConfigurationProperty(PRO_LOGICAL_DELETE_COLUMN);
    }
    if (introspectedTable.getTableConfigurationProperty(PRO_LOGICAL_DELETE_VALUE) != null) {
      this.logicalDeleteValue =
          introspectedTable.getTableConfigurationProperty(PRO_LOGICAL_DELETE_VALUE);
    }
    if (introspectedTable.getTableConfigurationProperty(PRO_LOGICAL_UN_DELETE_VALUE) != null) {
      this.logicalUnDeleteValue =
          introspectedTable.getTableConfigurationProperty(PRO_LOGICAL_UN_DELETE_VALUE);
    }
    // 3. 判断该表是否存在逻辑删除列
    ...
    for (IntrospectedColumn column : columns) {
      if (column.getActualColumnName().equalsIgnoreCase(logicalDeleteColumn)) {
        ...
        if (JDBCType.BIGINT == type || JDBCType.BIT == type || JDBCType.BOOLEAN == type
            || JDBCType.CHAR == type || JDBCType.DECIMAL == type || JDBCType.DOUBLE == type
            || JDBCType.FLOAT == type || JDBCType.INTEGER == type || JDBCType.LONGNVARCHAR == type
            || JDBCType.LONGVARCHAR == type || JDBCType.NCHAR == type || JDBCType.NUMERIC == type
            || JDBCType.NVARCHAR == type || JDBCType.SMALLINT == type || JDBCType.TINYINT == type
            || JDBCType.VARCHAR == type) {
          this.logicalDeleteColumn = column;
        } else {
          warnings.add("mybatis(逻辑删除插件):" + introspectedTable.getFullyQualifiedTable() + "逻辑删除列("
              + introspectedTable.getTableConfigurationProperty(PRO_LOGICAL_DELETE_COLUMN)
    ...
    if (introspectedTable.getTableConfigurationProperty(PRO_LOGICAL_DELETE_COLUMN) != null
        && this.logicalDeleteColumn == null) {
      warnings.add("mybatis(逻辑删除插件):" + introspectedTable.getFullyQualifiedTable() + "没有找到您配置的逻辑删除列("
          + introspectedTable.getTableConfigurationProperty(PRO_LOGICAL_DELETE_COLUMN) + ")！");
    }

    // 4. 判断逻辑删除值是否配置了
    if (this.logicalDeleteColumn != null
        && (this.logicalDeleteValue == null || this.logicalUnDeleteValue == null)) {
      warnings.add("mybatis(逻辑删除插件):" + introspectedTable.getFullyQualifiedTable()
          + "没有找到您配置的逻辑删除值，请全局或者局部配置logicalDeleteValue和logicalUnDeleteValue值！");
    }

    // 5. 获取逻辑删除常量值
    this.logicalDeleteConstName = properties.getProperty(PRO_LOGICAL_DELETE_CONST_NAME) != null
        ? properties.getProperty(PRO_LOGICAL_DELETE_CONST_NAME).toUpperCase()
        : DEFAULT_LOGICAL_DELETE_CONST_NAME;
    this.logicalUnDeleteConstName = properties.getProperty(PRO_LOGICAL_UN_DELETE_CONST_NAME) != null
        ? properties.getProperty(PRO_LOGICAL_UN_DELETE_CONST_NAME).toUpperCase()
        : DEFAULT_LOGICAL_UN_DELETE_CONST_NAME;

    // 6. 防止增强的selectByPrimaryKey中逻辑删除键冲突
    if (this.logicalDeleteColumn != null) {
      Field logicalDeleteField =
          JavaBeansUtil.getJavaBeansField(this.logicalDeleteColumn, context, introspectedTable);
      if (logicalDeleteField.getName().equals(PARAMETER_LOGICAL_DELETED)) {
        this.logicalDeleteColumn = null;
        warnings.add("mybatis(逻辑删除插件):" + introspectedTable.getFullyQualifiedTable()
            + "配置的逻辑删除列和插件保留关键字(" + PARAMETER_LOGICAL_DELETED + ")冲突！");
      }
    }
  }
  ...
  public boolean clientGenerated(Interface interfaze, TopLevelClass topLevelClass,
    ...
    if (this.logicalDeleteColumn != null) {
      // 1. 逻辑删除ByExample
      Method mLogicalDeleteByExample =
          JavaElementGeneratorTools.generateMethod(METHOD_LOGICAL_DELETE_BY_EXAMPLE,
      ...
      logger.debug(
          "mybatis(逻辑删除插件):" + interfaze.getType().getShortName() + "增加方法logicalDeleteByExample。");

      // 2. 判断是否有主键，生成主键删除方法
      if (introspectedTable.hasPrimaryKeyColumns()) {
        // 2.1. 逻辑删除ByExample
        Method mLogicalDeleteByPrimaryKey =
            JavaElementGeneratorTools.generateMethod(METHOD_LOGICAL_DELETE_BY_PRIMARY_KEY,
                JavaVisibility.DEFAULT, FullyQualifiedJavaType.getIntInstance());
        ...
        Method mSelectByPrimaryKey = JavaElementGeneratorTools.generateMethod(
            METHOD_SELECT_BY_PRIMARY_KEY_WITH_LOGICAL_DELETE, JavaVisibility.DEFAULT,
        ...
        logger.debug("mybatis(逻辑删除插件):" + interfaze.getType().getShortName()
            + "增加方法logicalDeleteByPrimaryKey。");

        // 增强selectByPrimaryKey
        mSelectByPrimaryKey
            .addParameter(new Parameter(FullyQualifiedJavaType.getBooleanPrimitiveInstance(),
                PARAMETER_LOGICAL_DELETED, "@Param(\"" + PARAMETER_LOGICAL_DELETED + "\")"));
  ...
  public boolean sqlMapDocumentGenerated(Document document, IntrospectedTable introspectedTable) {
    if (this.logicalDeleteColumn != null) {
      ...
      if (this.logicalDeleteValue == null || "NULL".equalsIgnoreCase(this.logicalDeleteValue)) {
        sb.append("NULL");
      } else if (JDBCType.CHAR == type || JDBCType.LONGNVARCHAR == type
          || JDBCType.LONGVARCHAR == type || JDBCType.NCHAR == type || JDBCType.NVARCHAR == type
          || JDBCType.VARCHAR == type) {
        sb.append("'");
      ...
      logger.debug("mybatis(逻辑删除插件):" + introspectedTable.getMyBatis3XmlMapperFileName()
      ...
      if (introspectedTable.hasPrimaryKeyColumns()) {
        ...
        logicalDeleteByPrimaryKey
            .addAttribute(new Attribute("id", METHOD_LOGICAL_DELETE_BY_PRIMARY_KEY));
        ...
        if (this.logicalDeleteValue == null || "NULL".equalsIgnoreCase(this.logicalDeleteValue)) {
          sb1.append("NULL");
        } else if (JDBCType.CHAR == type1 || JDBCType.LONGNVARCHAR == type1
            || JDBCType.LONGVARCHAR == type1 || JDBCType.NCHAR == type1
            || JDBCType.NVARCHAR == type1 || JDBCType.VARCHAR == type1) {
          sb1.append("'");
        ...
        selectByPrimaryKey
            .addAttribute(new Attribute("id", METHOD_SELECT_BY_PRIMARY_KEY_WITH_LOGICAL_DELETE));
        if (introspectedTable.getRules().generateResultMapWithBLOBs()) {
          selectByPrimaryKey.addAttribute(
              new Attribute("resultMap", introspectedTable.getResultMapWithBLOBsId()));
        } else {
          selectByPrimaryKey
        ...
        sb.append("  and ");
        sb.append(
            MyBatis3FormattingUtilities.getAliasedEscapedColumnName(this.logicalDeleteColumn));
        sb.append(" = ");
        selectByPrimaryKey.addElement(new TextElement(sb.toString()));
  ...
  /**
   * 生成Model 逻辑删除相关方法和常量 !!! bugfix：逻辑删除列作为复合主键的一部分的情况
   * 
   * @param topLevelClass
   * @param introspectedTable
   */
  private void generateModelMethodsAndFields(TopLevelClass topLevelClass,
      IntrospectedTable introspectedTable) {
    if (this.logicalDeleteColumn != null) {
      // 常量和逻辑删除方法跟随 逻辑删除列走
      boolean flag = false;
      ...
      if (flag) {
        ...
        } else if (this.logicalDeleteColumn.getFullyQualifiedJavaType()
            .getShortNameWithoutTypeArguments().equalsIgnoreCase("String")) {
          logicalDeleteValue = "\"" + this.logicalDeleteValue + "\"";
        } else if (this.logicalDeleteColumn.getFullyQualifiedJavaType()
            .getShortNameWithoutTypeArguments().equalsIgnoreCase("Boolean")) {
          logicalDeleteValue = (this.logicalDeleteValue.equals("1")
              || this.logicalDeleteValue.equalsIgnoreCase("true")) ? "true" : "false";
        } else {
          logicalDeleteValue = this.logicalDeleteValue;
        }
        // TODO 过期
        Field field = JavaElementGeneratorTools.generateStaticFinalField("DEL_FLAG_OFF",
            this.logicalDeleteColumn.getFullyQualifiedJavaType(), logicalDeleteValue);
        field.addAnnotation("@Deprecated");
        commentGenerator.addFieldComment(field, introspectedTable);
        // 常量插入到第一位
        fields.add(0, field);

        Field logicalDeleteConstField =
            JavaElementGeneratorTools.generateStaticFinalField(this.logicalDeleteConstName,
                this.logicalDeleteColumn.getFullyQualifiedJavaType(), logicalDeleteValue);
        ...
        if (this.logicalUnDeleteValue == null
            || "NULL".equalsIgnoreCase(this.logicalUnDeleteValue)) {
          ...
          logicalUnDeleteValue = "\"" + this.logicalUnDeleteValue + "\"";
            ...
            .getShortNameWithoutTypeArguments().equalsIgnoreCase("Boolean")) {
          logicalUnDeleteValue = (this.logicalUnDeleteValue.equals("1")
              || this.logicalUnDeleteValue.equalsIgnoreCase("true")) ? "true" : "false";
        } else {
          logicalUnDeleteValue = this.logicalUnDeleteValue;
        }

        // TODO 过期
        Field field1 = JavaElementGeneratorTools.generateStaticFinalField("DEL_FLAG_ON",
            this.logicalDeleteColumn.getFullyQualifiedJavaType(), logicalUnDeleteValue);
        field1.addAnnotation("@Deprecated");
        commentGenerator.addFieldComment(field1, introspectedTable);
        // 常量插入到第一位
        fields.add(0, field1);

        Field logicalUnDeleteConstField =
            JavaElementGeneratorTools.generateStaticFinalField(this.logicalUnDeleteConstName,
                this.logicalDeleteColumn.getFullyQualifiedJavaType(), logicalUnDeleteValue);
        ...
        // ================================================= andLogicalDeleted
        // =============================================
        Method mAndLogicalDeleted = JavaElementGeneratorTools.generateMethod(METHOD_LOGICAL_DELETED,
        ...
        Method logicalDeleteSetter =
            JavaBeansUtil.getJavaBeansSetter(this.logicalDeleteColumn, context, introspectedTable);
        mAndLogicalDeleted.addBodyLine(logicalDeleteSetter.getName() + "(deleted ? "
            + this.logicalDeleteConstName + " : " + this.logicalUnDeleteConstName + ");");
    ...
    if (this.logicalDeleteColumn != null) {
      ...
      for (InnerClass innerClass : innerClasses) {
        if ("Criteria".equals(innerClass.getType().getShortName())) {
          ...
          logger.debug("mybatis(逻辑删除插件):" + topLevelClass.getType().getShortName() + "."
              + innerClass.getType().getShortName() + "增加andDeleted方法。");
        }
      }
    }
    return true;
