...
public class stringPreHandlingModule {
  ...
  public static String numberTranslator(String target) {
    Pattern p = Pattern.compile("[一二两三四五六七八九123456789]万[一二两三四五六七八九123456789](?!([千百十]))");
    Matcher m = p.matcher(target);
    StringBuffer sb = new StringBuffer();
    boolean result = m.find();
    while (result) {
    ...
    p = Pattern.compile("[一二两三四五六七八九123456789]千[一二两三四五六七八九123456789](?!([百十]))");
    m = p.matcher(target);
    sb = new StringBuffer();
    result = m.find();
    while (result) {
    ...
    p = Pattern.compile("[零〇○Ｏ０OoΟ0О一二两三四五六七八九]");
    m = p.matcher(target);
    sb = new StringBuffer();
    result = m.find();
    while (result) {
  ...
  private static int wordToNumber(String s) {
    String strZero = "零〇○Ｏ０OoΟ0О0";
    // if (s.equals("零") || s.equals("0"))
    if (strZero.contains(s))
      return 0;
