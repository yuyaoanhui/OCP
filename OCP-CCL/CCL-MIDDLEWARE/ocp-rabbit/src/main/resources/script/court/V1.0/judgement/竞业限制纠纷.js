var NAMESPACE_COURT_JUDGEMENT_竞业限制纠纷 = {
	/**
	 * point-1:劳动合同签订
	 */
	 ...
	 * point-9:离职后竞业限制时间
	 */
	info_non_competitive_time : {
		params : [ {
			tagList : "court_opinion",
			regex : "(离职|解除劳动关系|合同终止)[^,，\\.。;；、]*(\\d|\\W)年",
            reverse : "0",
			unit : "年"
		}, {
			tagList : "court_opinion",
			regex : "(\\d|\\W)年[^,，\\.。;；]*(同类|同(行?)业|竞争|有关|相关)",
            reverse : "0",
			unit : "年"
		} ],
		method : "rule_func_extract_time",
	 ...
	 * point-10:违约金
	 */
	info_liquidated_damages : {
		params : [ {
			tagList : "judgement_content",
			regex : "违约金[^,，\\.。;；、]*([\\d\\.]+)(万?)元|([\\d\\.]+)(万?)元[^,，\\.。;；、]*违约金"
		} ],
		method : "rule_func_extract_money",
		adjust : [ {
			params : [ {
				valueNodes : "0#1#5#10#50",
				valueRanges : "0-1万#1-5万#5-10万#10-50万#50万以上"
			} ],
			method : "rule_func_adjust_value_to_range"
		} ]
	}
